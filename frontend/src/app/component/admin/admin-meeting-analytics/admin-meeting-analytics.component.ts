...
export class AdminMeetingAnalyticsComponent implements OnInit {
  ...
  generatePDFReport(subject: SubjectGroup) {
    ...
    doc.text(`Admin Subject Analytics Report`, pageWidth / 2, 20, { align: 'center' });
    ...
    const summaryData = [
      ...
      ['Number of Lecturers', subject.lecturers.size.toString()],
    ...
    // Lecturers List
    const lecturersY = (doc as any).lastAutoTable.finalY || finalY + 20;
    
    if (lecturersY + 30 > doc.internal.pageSize.height - 20) {
      doc.addPage();
      doc.setFontSize(14);
      doc.text('Lecturers Teaching This Subject', 14, 20);
      let startY: number = 25;
      
      const lecturersData = Array.from(subject.lecturers).map(l => [l]);
      autoTable(doc, {
        startY: startY,
        head: [['Lecturer Name']],
        body: lecturersData,
      ...
      doc.setFontSize(14);
      doc.text('Lecturers Teaching This Subject', 14, lecturersY + 15);
      
      const lecturersData = Array.from(subject.lecturers).map(l => [l]);
      autoTable(doc, {
        startY: lecturersY + 20,
        head: [['Lecturer Name']],
        body: lecturersData,
        theme: 'striped',
        headStyles: { fillColor: [25, 118, 210] }
    ...
    // Meetings List
    const meetingsY = (doc as any).lastAutoTable.finalY || lecturersY + 20;
    ...
    autoTable(doc, {
      startY: startY,
      head: [['Topic', 'Date', 'Lecturer', 'Status', 'Attendance', 'Emotions']],
      body: meetingsData,
      theme: 'striped',
      headStyles: { fillColor: [25, 118, 210] },
      styles: { fontSize: 8 }
    ...
    // Save PDF
    const fileName = `${subject.subjectName.replace(/[^a-z0-9]/gi, '_')}_Admin_Report_${new Date().getTime()}.pdf`;
    doc.save(fileName);
  ...
  generateCSVReport(subject: SubjectGroup) {
    ...
    // Header
    csvContent += `Admin Subject Analytics Report\n`;
    ...
    csvContent += `Number of Lecturers,${subject.lecturers.size}\n`;
    ...
    // Lecturers List
    csvContent += `Lecturers Teaching This Subject\n`;
    csvContent += `Lecturer Name\n`;
    Array.from(subject.lecturers).forEach(lecturer => {
      csvContent += `"${lecturer.replace(/"/g, '""')}"\n`;
    });
    csvContent += `\n`;
    
    // Meetings Breakdown
    csvContent += `Meetings Breakdown\n`;
    csvContent += `Topic,Date,Lecturer,Status,Attendance Rate,Emotion Records\n`;
    ...
    const fileName = `${subject.subjectName.replace(/[^a-z0-9]/gi, '_')}_Admin_Report_${new Date().getTime()}.csv`;
